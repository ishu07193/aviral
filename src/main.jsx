...
const App = () => {
  ...
  useEffect(() => {
    const widget = new window.TradingView.widget({
      ...
      symbol: "CBOE:AAPL", // You can change this to BTCUSD or any symbol
      interval: "1",
      theme: "light",
      style: "1",
      locale: "en",
    ...
    widget.onChartReady(() => {
      ...
      widget.activeChart().onDataLoaded().subscribe(null, () => {
        const priceSeries = widget.activeChart().createSeries({
          name: symbol,
          type: "line",
        });

        widget.activeChart().onSymbolChanged().subscribe(null, () => {
          setPrice(0); // Reset on symbol change
        });

        // Realtime price update logic
        widget.activeChart().onRealtimeUpdate().subscribe(null, () => {
          ...
          if (lastBar && lastBar.v && lastBar.v[4]) {
            setPrice(lastBar.v[4]); // Close price
          }
        });
      });
    });
